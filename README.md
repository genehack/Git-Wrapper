...
- error
    ...
    `STDERR`. This method should not be used as a success/failure check, as
...
- output
    ...
    `STDOUT`.  This method should not be used as a success/failure check, as
    `git` will frequently not have any output with a successful command.
...
[Try::Tiny](https://metacpan.org/pod/Try::Tiny) is the recommended way to catch exception objects thrown by
...
If for some reason you are unable to use [Try::Tiny](https://metacpan.org/pod/Try::Tiny), it is also possible to use the `eval` function to catch exception objects. **THIS IS NOT RECOMMENDED!**
